...
import { Mail, Save, CheckCircle, AlertCircle, Send, Eye, EyeOff, Gift } from 'lucide-react';
...
export function EmailSettings() {
  ...
  const loadEmailSettings = async () => {
    const { data } = await supabase
      ...
      .in('key', [
        ...
        'payment_email_subject', 'payment_email_body', 'payment_email_body_html',
      ...
      data.forEach(setting => {
        switch (setting.key) {
          ...
          case 'gift_card_email_from_name':
            setGiftCardEmailFromName(setting.value);
            break;
          case 'gift_card_email_subject':
            setGiftCardEmailSubject(setting.value);
            break;
          case 'gift_card_email_body_text':
            setGiftCardEmailBodyText(setting.value);
            break;
          case 'gift_card_email_body_html':
            setGiftCardEmailBodyHtml(setting.value);
            break;
        }
      });
    }
  ...
  const insertGiftCardVariable = (variable: string) => {
    if (activeGiftCardTab === 'html') {
      ...
      if (textarea) {
        ...
        const text = giftCardEmailBodyHtml;
      ...
      if (textarea) {
        const start = textarea.selectionStart;
        const end = textarea.selectionEnd;
        const text = giftCardEmailBodyText;
        const before = text.substring(0, start);
        const after = text.substring(end);
  ...
  const handleTestGiftCardEmail = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setTestGiftCardResult({
          success: true,
          message: `Test-Gutschein-E-Mail erfolgreich an ${testEmail} gesendet! Bitte prüfen Sie Ihr Postfach.`
        });
      } else {
        setTestGiftCardResult({
          success: false,
      ...
      console.error('Test gift card email error:', error);
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-800 rounded-xl p-6 border border-slate-700 space-y-6">
        ...
        <div className="flex items-start space-x-4">
          ...
          <div className="flex-1 space-y-6">
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-2">
                Absendername (Gutschein)
              </label>
              <p className="text-xs text-slate-400 mb-3">
                Der Name, der als Absender in der Gutschein-E-Mail erscheint
              </p>
              <input
                type="text"
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  type="button"
                  onClick={() => insertGiftCardVariable('{{recipient_name}}')}
                  className="px-3 py-1.5 bg-amber-600 hover:bg-amber-700 text-white rounded text-sm transition"
                >
                  {'{{recipient_name}}'}
                </button>
                <button
                  type="button"
                  onClick={() => insertGiftCardVariable('{{purchaser_name}}')}
                  className="px-3 py-1.5 bg-amber-600 hover:bg-amber-700 text-white rounded text-sm transition"
                >
                  {'{{purchaser_name}}'}
                </button>
                <button
                  type="button"
                  onClick={() => insertGiftCardVariable('{{code}}')}
                  className="px-3 py-1.5 bg-amber-600 hover:bg-amber-700 text-white rounded text-sm transition"
                >
                  {'{{code}}'}
                </button>
                <button
                  type="button"
                  onClick={() => insertGiftCardVariable('{{barcode}}')}
                  className="px-3 py-1.5 bg-amber-600 hover:bg-amber-700 text-white rounded text-sm transition"
                >
                  {'{{barcode}}'}
                </button>
                <button
                  type="button"
                  onClick={() => insertGiftCardVariable('{{amount}}')}
                  className="px-3 py-1.5 bg-amber-600 hover:bg-amber-700 text-white rounded text-sm transition"
                >
                  {'{{amount}}'}
                </button>
                <button
                  type="button"
                  onClick={() => insertGiftCardVariable('{{expiry_date}}')}
                  className="px-3 py-1.5 bg-amber-600 hover:bg-amber-700 text-white rounded text-sm transition"
                >
                  {'{{expiry_date}}'}
                </button>
                <button
                  type="button"
                  onClick={() => insertGiftCardVariable('{{message}}')}
                  className="px-3 py-1.5 bg-amber-600 hover:bg-amber-700 text-white rounded text-sm transition"
                >
                  {'{{message}}'}
                </button>
                <button
                  type="button"
                  onClick={() => insertGiftCardVariable('{{pdf_url}}')}
                  className="px-3 py-1.5 bg-amber-600 hover:bg-amber-700 text-white rounded text-sm transition"
                >
                  {'{{pdf_url}}'}
                </button>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="block text-sm font-medium text-slate-300">
                  E-Mail-Inhalt (Gutschein)
                </label>
                <div className="flex items-center space-x-3">
                  {activeGiftCardTab === 'html' && (
                    <button
                      type="button"
                      onClick={() => setShowGiftCardPreview(!showGiftCardPreview)}
                      className="flex items-center space-x-2 px-3 py-1.5 bg-slate-900 border border-slate-700 rounded-lg text-slate-300 hover:text-white hover:border-slate-600 transition text-sm"
                    >
                      {showGiftCardPreview ? (
                        <>
                  ...
                  <div className="flex bg-slate-900 rounded-lg p-1 border border-slate-700">
                    <button
                      type="button"
                      onClick={() => setActiveGiftCardTab('text')}
                      className={`px-4 py-1.5 text-sm font-medium rounded transition ${
                    ...
                    <button
                      type="button"
                      onClick={() => setActiveGiftCardTab('html')}
                      className={`px-4 py-1.5 text-sm font-medium rounded transition ${
                        activeGiftCardTab === 'html'
                          ? 'bg-amber-600 text-white'
                          : 'text-slate-400 hover:text-white'
              ...
              {activeGiftCardTab === 'text' ? (
                <>
                  <p className="text-xs text-slate-400 mb-3">
                    Einfacher Text für die Gutschein-E-Mail (wird als Fallback verwendet, wenn HTML nicht angezeigt werden kann)
                  </p>
                  <textarea
                ...
                <>
                  <p className="text-xs text-slate-400 mb-3">
                    Vollständiger HTML-Code für die Gutschein-E-Mail. Verwenden Sie table-basiertes Layout für beste E-Mail-Client-Kompatibilität.
                  </p>
      ...
      <div className="bg-amber-900/20 border border-amber-800 rounded-xl p-6">
        ...
        <p className="text-sm text-slate-300 mb-4">
          Diese Platzhalter können in Gutschein-E-Mails verwendet werden:
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
          <div className="bg-slate-900 rounded-lg p-3">
            <code className="text-amber-400">{'{{recipient_name}}'}</code>
            <p className="text-slate-400 text-xs mt-1">Name des Empfängers</p>
          </div>
          <div className="bg-slate-900 rounded-lg p-3">
            <code className="text-amber-400">{'{{purchaser_name}}'}</code>
          ...
          <div className="bg-slate-900 rounded-lg p-3">
            <code className="text-amber-400">{'{{barcode}}'}</code>
            <p className="text-slate-400 text-xs mt-1">URL zum Barcode-Bild</p>
          </div>
          <div className="bg-slate-900 rounded-lg p-3">
            <code className="text-amber-400">{'{{amount}}'}</code>
            <p className="text-slate-400 text-xs mt-1">Gutscheinwert</p>
          </div>
          <div className="bg-slate-900 rounded-lg p-3">
            <code className="text-amber-400">{'{{expiry_date}}'}</code>
          ...
          <div className="bg-slate-900 rounded-lg p-3">
            <code className="text-amber-400">{'{{pdf_url}}'}</code>
      ...
      <div className="bg-green-900/20 border border-green-800 rounded-xl p-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          />
          <button
            onClick={handleTestGiftCardEmail}
            disabled={testingGiftCardEmail}
            className="px-6 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition disabled:opacity-50 flex items-center justify-center space-x-2 whitespace-nowrap"
          >
            <Send className="w-4 h-4" />
        ...
        {testGiftCardResult && (
          <div className={`mt-4 p-4 rounded-lg ${testGiftCardResult.success ? 'bg-green-900/30 border border-green-700' : 'bg-red-900/30 border border-red-700'}`}>
            <p className={`text-sm ${testGiftCardResult.success ? 'text-green-300' : 'text-red-300'}`}>
              {testGiftCardResult.message}
            </p>
          </div>
        )}
      </div>
