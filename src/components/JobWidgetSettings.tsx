...
export function JobWidgetSettings() {
  const [copied, setCopied] = useState<string | null>(null);

  const origin = window.location.origin;
  const widgetUrl = `${origin}/jobs-widget.html`;

  const iframeCode = `<!-- Offene Stellen Widget -->
<iframe
  src="${widgetUrl}"
...
></iframe>`;
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight mb-1">Widget Einbindung</h2>
        <p className="text-sm text-slate-400 dark:text-slate-500">Binde das Stellenanzeigen-Widget auf deiner Website ein</p>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-[#111113] rounded-2xl border border-slate-100 dark:border-white/5 p-5 shadow-sm space-y-4">
          ...
          <ol className="space-y-1.5 text-xs text-slate-500 dark:text-slate-400 list-decimal list-inside">
            ...
            <li>Widget erscheint automatisch mit aktuellen Stellen</li>
          </ol>
        </div>
      ...
      <div className="bg-white dark:bg-[#111113] rounded-2xl border border-slate-100 dark:border-white/5 p-5 shadow-sm">
        ...
        <ol className="space-y-3 text-sm text-slate-600 dark:text-slate-300">
          ...
          <li className="flex gap-3">
            ...
            <span>Neue Stellen in der Verwaltung hinzufügen — sie erscheinen automatisch auf der Website</span>
          </li>
        </ol>
      </div>
      ...
      <div className="bg-teal-50 dark:bg-teal-500/8 border border-teal-200/60 dark:border-teal-500/20 rounded-2xl p-5">
        ...
        <ul className="space-y-1.5 text-sm text-teal-700 dark:text-teal-400">
          <li className="flex items-start gap-2">
            ...
            <span>Stelle im Bereich "Offene Stellen" erstellen und aktivieren</span>
          </li>
          <li className="flex items-start gap-2">
            <Check className="w-4 h-4 shrink-0 mt-0.5 text-teal-500" />
            <span>Das Widget zeigt automatisch alle aktiven Stellen in Echtzeit an</span>
          </li>
          <li className="flex items-start gap-2">
            <Check className="w-4 h-4 shrink-0 mt-0.5 text-teal-500" />
            <span>Deaktivierte Stellen werden nicht angezeigt — kein Code-Update nötig</span>
          </li>
          <li className="flex items-start gap-2">
            <Check className="w-4 h-4 shrink-0 mt-0.5 text-teal-500" />
            <span>Bewerber können direkt per E-Mail oder über einen Link Kontakt aufnehmen</span>
          </li>
        </ul>
      </div>
    </div>
